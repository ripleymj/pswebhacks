...
final class JNAKernel32Library {
	...
	// LogManager.getLogger(JNAKernel32Library.class);
	...
	private JNAKernel32Library() {
		if (Constants.WINDOWS) {
			try {
				...
				doLog("windows/Kernel32 library loaded");
			} catch (NoClassDefFoundError e) {
				doLog("JNA not found. native methods and handlers will be disabled.");
			} catch (UnsatisfiedLinkError e) {
				doLog("unable to link Windows/Kernel32 library. native methods and handlers will be disabled.");
			}
		}
	}
	 ...
	 *             if the Kernel32 library is not loaded or if the native function
	 *             is not found
	...
	class NativeHandlerCallback implements StdCallLibrary.StdCallCallback {
		...
		public boolean callback(long dwCtrlType) {
			...
			doLog("console control handler receives event " + event + ", " + dwCtrlType);
	...
	/**
	 * Contains information about a range of pages in the virtual address space of a
	 * process. The VirtualQuery and VirtualQueryEx functions use this structure.
	 *
	...
	public static class MemoryBasicInformation extends Structure {
		...
		protected List<String> getFieldOrder() {
			return Arrays.asList("BaseAddress", "AllocationBase", "AllocationProtect", "RegionSize", "State", "Protect",
					"Type");
		}
	}
	 ...
	 * physical memory, ensuring that subsequent access to the region will not incur
	 * a page fault.
	 *
	 ...
	 *            queried.
	 * @param memoryInfo
	 *            A pointer to a structure in which information about the specified
	 *            page range is returned.
	 * @param length
	 *            The size of the buffer pointed to by the memoryInfo parameter, in
	 *            bytes.
	...
	/**
	 * Retrieves the short path form of the specified path. See <a href=
	 * "https://msdn.microsoft.com/en-us/library/windows/desktop/aa364989.aspx">{@code GetShortPathName}</a>.
	 *
	 * @param lpszLongPath
	 *            the path string
	 * @param lpszShortPath
	 *            a buffer to receive the short name
	 * @param cchBuffer
	 *            the size of the buffer
	 * @return the length of the string copied into {@code lpszShortPath}, otherwise
	 *         zero for failure
	 */
	...
	public static class JOBOBJECT_BASIC_LIMIT_INFORMATION extends Structure implements Structure.ByReference {
		...
		protected List<String> getFieldOrder() {
			return Arrays.asList("PerProcessUserTimeLimit", "PerJobUserTimeLimit", "LimitFlags",
					"MinimumWorkingSetSize", "MaximumWorkingSetSize", "ActiveProcessLimit", "Affinity", "PriorityClass",
					"SchedulingClass");
		}
	}

	/**
	 * Constant for JOBOBJECT_BASIC_LIMIT_INFORMATION in Query/Set
	 ...
	 *            job handle
	 * @param infoClass
	 *            information class constant
	 * @param info
	 *            pointer to information structure
	 * @param infoLength
	 *            size of information structure
	 * @param returnLength
	 *            length of data written back to structure (or null if not wanted)
	 * @return true if the function succeeds
	 */
	native boolean QueryInformationJobObject(Pointer job, int infoClass, Pointer info, int infoLength,
